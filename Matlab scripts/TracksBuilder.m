...
function TracksBuilder
    ...
    Settings = struct;
    Settings.PixelSize = 1; % in µm per pixel. Conversion not needed for this data...
    Settings.AcquisitionRate = 500; % Frames per second
    Settings.MaxVelocity = 300;% Filter for the particle velocity in µm per s
    Settings.MinTrackDuration = 30; % Filter to remove small tracks (in Frames)
    Settings.MinTrackExtension = 2; % Filter to remove short tracks (usually particles sticking to the surface) (in µm)
...
LengthVector = 2.5; % Arrow length in µm
