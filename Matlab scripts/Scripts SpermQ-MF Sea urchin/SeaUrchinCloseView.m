...
LocationReferenceArrow = [22 -6 0];
Draft = false; %Make true to make an simpler, faster, plot
Start = 1;
Step = 1;
Frame = Start:Step:Start + 25;
...
%% Plot colorbar
PlotColorbar('SeaUrchinCloseView', 'Time (ms)', 50)
...
text(LocationReferenceArrow(1),LocationReferenceArrow(2)+1,LocationReferenceArrow(3)+2.7,'10 µm','Rotation',90,'FontSize',24)
...
if Draft
    ...
    exportgraphics(gca, 'SeaUrchin_CloseView.png', 'ContentType', 'image', 'Resolution', 300)  
end
...
function PlotColorbar(FileName, Label, MaxTime)
    ...
    exportgraphics(gca, strcat(FileName, '_Close_Colorbar.pdf'), 'ContentType', 'vector')  
    close (FigColorbar)
end
